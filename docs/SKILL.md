...
Start the metrics exporter. Connects to ClickHouse, runs 13 collectors on a poll loop, exposes `/metrics` and `/healthz`. The Helm chart can install PrometheusRule alerts for replication queue backlog, missing-part replication failures, part explosions, disk fullness, Keeper health, Kafka failures, object storage failures, memory limits, and ClickHouse guardrail rejections.
