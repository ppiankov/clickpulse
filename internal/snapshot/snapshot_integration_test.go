...
var (
	snapshotIntegrationDB      *sql.DB
	snapshotIntegrationCleanup func()
	snapshotIntegrationErr     error
)

func TestMain(m *testing.M) {
	snapshotIntegrationDB, snapshotIntegrationCleanup, snapshotIntegrationErr = setupIntegrationDB(context.Background(), "snapshot")

	code := m.Run()

	if snapshotIntegrationDB != nil {
		_ = snapshotIntegrationDB.Close()
	}
	if snapshotIntegrationCleanup != nil {
		snapshotIntegrationCleanup()
	}
...
func TestSnapshotTake(t *testing.T) {
	...
	if s.Uptime <= 0 {
		t.Fatalf("snapshot uptime must be positive, got %s", s.Uptime)
	}
}
...
func requireIntegrationDB(t *testing.T) *sql.DB {
	...
	if snapshotIntegrationErr != nil {
		t.Skipf("integration ClickHouse unavailable: %v", snapshotIntegrationErr)
	}

	return snapshotIntegrationDB
}
